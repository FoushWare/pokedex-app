...
export async function fetchPokemonPage(page: number, limit = POKEMON_PER_PAGE): Promise<{ pokemon: PokemonCardData[]; total: number }> {
  ...
  const details = await Promise.all(list.results.map((_, i) => fetchPokemonDetail(offset + i + 1)));
  return {
    pokemon: details.map((d) => ({ id: d.id, name: d.name, sprite: d.sprites.other['official-artwork'].front_default })),
    total: list.count,
  };
}
